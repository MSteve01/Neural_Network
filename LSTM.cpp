		...
		for (int round = v.size() - 1; round >= 0; round--) {													// loop thougj eery time step
			...
			for (int i = 0; i < value.get_row(); i++) {															// loop though every output row
				for (int j = 0; j < value.get_row(); j++) {														// loop though every input row
					xO_weight_change[i][j] += check_nan(doutput_gate[i][0] * v[round][j][0],0.0) * learning_rate;				// comupute changing weight
					...
					next_dh[i][0] += check_nan(doutput_gate[i][0] * hO_weight_change[i][j], 0.0);								// comupute next time step output error
					...
					flow_gadient[round][i][0] += check_nan(doutput_gate[i][0] * xO_weight_change[i][j], 0.0);					// computer flow gadient
